...
bool ReCab::processRead(SamRecord& samRecord,int processtype, ReCab::quality_t& quality_strings){
    ...
    CigarCol cigarcol;
    cigarcol.CigarParse(samRecord.getCigar());
    cigarcol.GetClipLength_begin(samRecord.getCigar());
    ...
    String aligTypes="";
      for (int i = 0; i < cigarcol.size; i++)
        for (int j = 0; j < cigarcol.fieldCount[i]; j++)
          aligTypes += cigarcol.fieldTypes[i];

    ssequence = samRecord.getSequence();
    lssequence = strlen(ssequence);
	...
	//exclude dbSNP sites
	if(dbSNP[refpos]==true) continue;

    refBase = toupper(referenceGenome[refpos]);
    seqpos = i + offset2;
    readBase = toupper(ssequence[seqpos]);
	...
	if(cobs<5){
		if((seqpos>0) && ((seqpos<(aligTypes.Length()-1)))){
		  //cprebase
		  if(aligTypes[i-1] == 'M')
			prebase = toupper(ssequence[seqpos-1]);
		  if(aligTypes[i+1] == 'M')
			nexbase = toupper(ssequence[seqpos+1]);
		}
		else
		{
		  if(seqpos==0){
