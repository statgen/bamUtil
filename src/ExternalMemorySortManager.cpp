...
int ExternalMemorySortManager::MergeSort(/*std::vector<std::string> tmpFileList*/) {//reading from signle tmp file(TODO:or heap), output to fianl file
	...
	if(host->prevRec!=NULL)//we didnt pick out the record in local static variable in handlePairedRN functions
	{
        std::cerr << "Paired Read, " << host->prevRec->getReadName()
                  << " but couldn't find mate, so writing as "<<host->myNumMateFailures<<"th\t"
                  << "unpaired (single-ended)\n";
        host->myNumMateFailures++;
        host->writeFastQ(*host->prevRec, host->myUnpairedFile,host->myUnpairedFileNameExt);
        host->prevRec=NULL;
	}
...
int ExternalMemorySortManager::miniMergeSort(MateVectorByRN* tmpVector) { //reading from tmp file, ouput single to single tmp file (TODO:or heap)
	...
	masterTmpFileList[vectorIndex] = tmpVector->tmpFileNameBase
			+ std::to_string(vectorIndex) + "th.vector.all.tmp.bam";
